...
def query(args):
    ...
    # Estimating pair-wise distances
    if not args.a:
	shutil.copytree(os.path.join(args.O, sample), os.path.join(args.library, sample))
    if sample not in set(refs):
	refs.append(sample)
    ...
    # Initializing distance dataframe
    index = pd.MultiIndex.from_product([refs, refs], names=['sample', 'sample_2'])
    result_s = pd.DataFrame(columns=index)

    for result in results_dist:
        dist_output = result.get(9999999)
        result_s[(dist_output[0], dist_output[1])] = [repr(dist_output[2])]

    # Writing distances to file
    sys.stderr.write('[skmer] Writing to file...\n')
    result_dfm = pd.melt(result_s, value_name='distance')
    result_mat = result_dfm.pivot(index='sample', columns='sample_2', values='distance')
    result_mat.to_csv('{0}-{1}.txt'.format(args.o, sample.lower()), sep='\t', mode='w')
    if not args.a:
	shutil.rmtree(os.path.join(args.library, sample))
